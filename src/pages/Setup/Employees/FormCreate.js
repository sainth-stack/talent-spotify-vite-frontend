...
const link =
  "https://res.cloudinary.com/dbqm9svvp/raw/upload/v1688020404/talentspotifypics/Employee-Template_zvs7g0.csv";
  ...
  const handleFileUpload = async ({ data: employees, file, url }) => {
      ...
      return {
        personalInformation: {
          ...
          dateOfBirth: employee.dateOfBirth
            ? window
                .moment(new Date(getDate(employee.dateOfBirth)))
                .format("YYYY-MM-DD")
        ...
        employmentInformation: {
          ...
          inactiveDate: employee.inactiveDate
            ? window
                .moment(new Date(getDate(employee.inactiveDate)))
                .format("YYYY-MM-DD")
            : null,
    ...
    let result = await axios
      .post(
        ...
        { data: finalEmployees },
        {
          onUploadProgress: (data) => {
            setTotal(totalEmployees.length);
            setLoaded(
              Math.round(
                100 * (data.loaded / data.total) * (totalEmployees.length / 100)
              )
            );
      ...
      .catch((err) => {
        ...
        reqBody.loadedData = filteredEmails.length;
        reqBody.totalData = totalEmployees.length;
