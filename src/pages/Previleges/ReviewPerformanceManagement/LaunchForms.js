...
export default function LaunchForms() {
  ...
  const handleSubmit = () => {
    ...
    templateInfo.displaySteps = templateData.filter((item) => item._id === templateInfo?.formTemplate)[0]?.displaySteps
    const finData = [];
    const mainData = templateInfo?.displaySteps.map((item, index) => {
      if (item.isChecked) {
        finData.push({
          id: finData.length,
          label: item.value
    ...
    response.then(({ success, message }) => {
      if (success) {
        let reqBody = templateInfo.employees.map((employee, index) => {
          const managerId = employeeResponse.data?.filter((item) => item._id === employee)[0]
          let reqBody = {
            employeeName: finData[0]?.label === "Submit" ? employee : '',
            employeeId:employee,
            ...
            competencies: [],
            managerId:JSON.parse(localStorage.getItem("companyId")) === "65901f8a16fcb600093e8f89" ? "": managerId?.employmentInformation?.lineManager,
            managerName: '',
            attachment: "",
            status: finData[0]?.label || "Submit",
      ...
      if (success) {
        let reqBody = templateInfo.employees.map((employee, index) => {
          ...
          let reqBody = {
            ...
            endDate: templateInfo.reviewPeriodEndDate,
            competencies: [],
            managerId: managerId?.employmentInformation?.lineManager,
            managerName: '',
            attachment: "",
            status: finData[0]?.label || "Submit",
            templateName: templateInfo.templateName,
