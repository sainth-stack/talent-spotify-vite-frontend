    ...
    response.then(({ success, message, data }) => {
      if (success) {
        ...
        let groupByRatings = ratingInfo.ratingNumbers.map(item => {
          ...
          let employeeLength = filtered.length;
          return { rating: item, numberOfRatings: employeeLength, employees: formattedEmployees }
        });
        let totalEmployees = sessionInfo.employees.length;
        let groupByPercentages = groupByRatings.map(item => {
          let percentage = Number((Number(item.numberOfRatings) / Number(totalEmployees)) * 100).toFixed(2);
          return {
            ...item,
            percentage
          }
        ...
        let result = {
          totalEmployees: sessionInfo.employees.length,
          groupByRatings: groupByRatings.map(item => item.numberOfRatings),
          empPercentages: groupByPercentages.map(item => item.percentage),
    ...
    <div className='p-3'>
      ...
      {loading ? <LoadingIndicator /> :
        <>
          ...
          />
          {ratingInfo.ratingLabels.length > 0 && isRefreshed && <BinChart ratingLabels={ratingInfo.ratingLabels} values={ratingInfo.ratingValues} empPercentages={ratingInfo.empPercentages} />}
          {ratingInfo.employees.length > 0 && <Calibration employeesData={ratingInfo.employees} ratingLabels={ratingInfo.ratingLabels} setRefreshed={setRefreshed} status={status} setRefreshData={setRefreshData}/>}
          {ratingInfo.employees.length > 0 && status === "HR Review" && <div className='container d-flex justify-content-end'>
            <Button
