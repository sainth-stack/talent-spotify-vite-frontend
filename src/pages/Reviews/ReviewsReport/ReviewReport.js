...
export default function Reviews() {
  ...
  const getReviewData = (data, prevData) => {
    ...
    try {
      ...
      const averageScores = allCategories?.map(item => {
        const filterComp = prevData.competencies.filter((itemc) => itemc.competencyName == item)
        ...
        const prevEmpComp = filterComp.filter(item3 => item3.type === "employee")[0]?.Feedback;
        const managerScore = filterData.filter(item3 => item3.type === "manager")[0]?.averageScore;
        const prevManagerComp = filterComp.filter(item3 => item3.type === "manager")[0]?.Feedback;
        return {
          category: item,
          averageScore: average,
          selfScore,
          managerScore,
          prevEmpComp: prevEmpComp ? prevEmpComp : 0,
          prevManagerComp: prevManagerComp ? prevManagerComp : 0,
  ...
  const getAllReviews = async () => {
    try {
      let response = dispatch(getReviewFormByUserId(userId));
      response.then(({ data, message }) => {
        if (data !== undefined && Object.keys(data).length > 0) {
          const resonseFin = data
          console.log(data,'testingg')
          const res1 = dispatch(getReviewFormById(resonseFin[0]?._id));
          res1.then(({ data, message }) => {
            const res1Data = data
            const res2 = dispatch(getReviewFormById(resonseFin[0]?._id));
            res2.then(({ data }) => {
              getReviewData(res1Data, data)
            })
          })
