...
const Section4 = ({ data }) => {
  ...
  const determineProfileType = (values) => {
    ...
    // Determine high/medium/low for each category
    const disruptiveLevel = disruptive >= 40 ? 'high' : disruptive >= 30 ? 'medium' : 'low';
    const realWorldLevel = realWorld >= 40 ? 'high' : realWorld >= 30 ? 'medium' : 'low';
    const implementationLevel = implementation >= 40 ? 'high' : implementation >= 30 ? 'medium' : 'low';

    // Balanced profile check
    ...
    if (Math.abs(disruptive - realWorld) < 10 && Math.abs(disruptive - implementation) >= 10) {
      setProfileType('equalDisruptiveRealWorld');
      return;
    }
    if (Math.abs(disruptive - implementation) < 10 && Math.abs(disruptive - realWorld) >= 10) {
      setProfileType('equalDisruptiveImplementation');
      return;
    }
    if (Math.abs(realWorld - implementation) < 10 && Math.abs(realWorld - disruptive) >= 10) {
      setProfileType('equalRealWorldImplementation');
      return;
    }

    // Standard combinations
    setProfileType(`${disruptiveLevel}Disruptive-${realWorldLevel}RealWorld-${implementationLevel}Implementation`);
  };

  const getContent = () => {
    const candidateName = data?.name || '[Candidate Name]';
