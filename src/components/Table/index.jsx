...
const MyExportCSV = (props) => {
  const handleClick = () => {
    if (
      props.data.length > 0 &&
      Object.keys(props.data[0]).includes("mainTask")
    ) {
      downloadTasksExcel(props.data);
    } else {
      props.onExport();
    }
  };
  return (
    <div className="justify-content-end d-none">
      <button className="btn btn-success" id="exportcsv" onClick={handleClick}>
        Export
      </button>
    </div>
  );
