...
class Calendar extends Component {
  ...
  componentDidMount() {
    const events = this.props.tasks.map((item) => {
      return {
        id: item.id,
        start: window.moment(item.startDate ? item.startDate : new Date()).startOf("day").format("YYYY-MM-DDTHH:mm:ss"),
        end: window.moment(item.startDate ? item.startDate : new Date()).endOf('day').format("YYYY-MM-DDTHH:mm:ss"),
        end: window.moment(item.dueDate ? item.dueDate : new Date()).endOf('day').format("YYYY-MM-DDTHH:mm:ss"),
        completionDate: window.moment(item.actualCompletionDate ? item.actualCompletionDate : new Date()).format("YYYY-MM-DDTHH:mm:ss"),
        backColor: item.status === "completed" ? "#2A7A7B" : (item.status === "inprogress" ? "orange" : "#FF0000"),
        image: this.props.employees && item.assignTo.map((employee) => (
          <img src={this.props.employees && this.props.employees.length > 0 ? this.props.employees.filter(item => item.value === employee).length > 0 ? this.props.employees.filter(item => item.value === employee)[0].profilePicture : defaultProfilePic : defaultProfilePic} alt="Avatar" title={this.props.employees && this.props.employees.length > 0 ? this.props.employees.filter(item => item.value === employee).length > 0 ? this.props.employees.filter(item => item.value === employee)[0].key : "" : ""} className="profile-pic2" />
        )),
        html: `<div title=${this.props.employees && this.props.employees.length > 0 ? item.assignTo.map((employee) => this.props.employees.filter(item => item.value === employee).length > 0 ? this.props.employees?.filter(item => item.value === employee)[0].key : "") : ""}><img src=${defaultProfilePic} width=${20} height=${20}/> ${item.title}</div>`
      }
    }
    )
    // [
